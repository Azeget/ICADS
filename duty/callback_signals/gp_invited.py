...
FAILED_MSG = (
    '❌ Не удалось  назначить группу администратором.\n'
...
def groupbot(event: Event):
    ...
    msg_id = event.api.msg_op(
        1, peer_id, '👀 Обнаружена группа Ириса...'
    )
    try:
        if event.api('messages.setMemberRole', peer_id=peer_id,
                     member_id=group_id, role='admin') == 1:
            event.api.msg_op(
                2, peer_id, '✅ Ирис назначен админом', msg_id
            )
    except VkApiResponseException as e:
        if e.error_code == 15:
            event.api.msg_op(2, peer_id, FAILED_MSG, msg_id)
        else:
            event.api.msg_op(2, peer_id, f'❗ Ошибка на стороне VK: {e.error_msg}', msg_id)
